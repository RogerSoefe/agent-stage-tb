var appCtrl = appModule.controller("appController", ['$rootScope', '$scope', '$agentService', '$webSocketService', '$route', '$translatorService', function ($rootScope, $scope, $agentService, $webSocketService, $route, $translatorService) {
    ...
    $rootScope.LeftMenuVisible = true;
  ...
  * */
  ...
  $scope.GetAgentHierarchy = function () {
	...
	jQuery(document).ready(function () {
		...
		var xDown = null;
		var yDown = null;
         var touchStartedOnSidebar = false;

		function handleTouchStart(evt) {
            if (!$rootScope.IsMobile || $scope.gridMode.isGrid) return;
            
            // Detectar si el touch comenzó sobre el sidebar o cerca del borde izquierdo
            var sidebar = document.getElementById('sidebar-wrapper');
            var touchX = evt.touches[0].clientX;
            var sidebarWidth = sidebar ? sidebar.offsetWidth : 250;
            var edgeThreshold = 50; // Píxeles desde el borde izquierdo para activar
            
            // Permitir swipe si:
            // 1. El sidebar está visible (toggled) y el touch está dentro del sidebar
            // 2. El sidebar está oculto y el touch está cerca del borde izquierdo
            var sidebarVisible = jQuery("#wrapper").hasClass("toggled");
            touchStartedOnSidebar = sidebarVisible ? (touchX <= sidebarWidth) : (touchX <= edgeThreshold);
            
            if (touchStartedOnSidebar) {
                xDown = touchX;
      ...
      if ($scope.IsActive('Maintenance') || $scope.IsActive('CustomerMapActivity') || $scope.gridMode.isGrid || $scope.IsActive('CustomerCounter') || ($scope.IsActive('Daily') && $rootScope.viewMode == 1)) return;
      
      // Solo procesar si el touch comenzó en el área del sidebar
      if (!touchStartedOnSidebar || !xDown || !yDown || !$rootScope.IsMobile) {
        return;
      }
      
			var hasSwipe = (evt.target.offsetParent.className.toString().indexOf("tablesaw-swipe") >= 0
        || evt.target.offsetParent.offsetParent.className.toString().indexOf("tablesaw-swipe") >= 0);
      if (hasSwipe) {
        // Reset si estamos en un elemento con scroll horizontal
        xDown = null;
        yDown = null;
      ...
      // Solo actuar si el movimiento horizontal es mayor que el vertical (swipe horizontal)
      if (Math.abs(xDiff) > Math.abs(yDiff) && Math.abs(xDiff) > 30) {/*most significant + mínimo de 30px*/
				if (xDiff > 0) {
    ...
    function handleClickOutside(evt) {
      ...
      // Solo actuar si el sidebar está visible
      if (!jQuery("#wrapper").hasClass("toggled")) return;
      ...
      if (sidebar && !sidebar.contains(target)) {
        // Evitar cerrar si se hace click en elementos que abren modals, bottom sheets, o el botón del menú
        var isExcluded = target.closest('.fab-button, .bottom-sheet, .modal, .swal2-container, #menu-toggle');
        if (!isExcluded) {
          $scope.ToggleMenu();
          $scope.$apply();
        }
    ...
    // Function to detect if device is tablet (Bootstrap md breakpoint)
    // Tablets: 768px (md) to 1199px (before lg)
  ...
  $scope.ToggleMenu = function () {
    if ($rootScope.LeftMenuVisible == true) {
    jQuery("#wrapper").toggleClass("toggled");
      jQuery("#div_dashbox").toggleClass("toggled");
      jQuery("#scrollBox").toggleClass("toggled");
      jQuery('.sub-sav-stack.active').removeClass('active');
      togglelList = [];
    } else {
  ...
  var Scheduling = setTimeout(function tick() {
    // Evitar múltiples llamadas si ya está en logout
    if (!window._isLoggingOut) {
      $agentService.TrackStatus()
        .then(function(response) {
          // Success - continuar el scheduling
          Scheduling = setTimeout(tick, SETTINGS.TrackTime);
        })
        .catch(function(error) {
          // Error de red o servidor - no forzar logout, reintentar
          console.warn('TrackStatus failed, retrying...', error);
          Scheduling = setTimeout(tick, SETTINGS.TrackTime);
        });
    }
  }, SETTINGS.TrackTime);
