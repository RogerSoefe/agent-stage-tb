appModule.controller("webAccessController", [
	'$scope', '$agentService', '$compile', '$rootScope', 'DTOptionsBuilder', 'DTColumnDefBuilder', '$http', function ($scope, $agentService, $compile, $rootScope, DTOptionsBuilder, DTColumnDefBuilder, $http) {
		let vm = this;
		$scope.OperationFilterList = [
			{ name: $scope.Translate('All'), value: 1 },
			{ name: $scope.Translate('Accept'), value: 2 },
			{ name: $scope.Translate('Viewed report'), value: 3 },
			{ name: $scope.Translate('Login'), value: 4 },
		];
		$scope.IpInfo = {};		
		let rawData = [];
		vm.dtInstance = {};
		vm.dtOptions = DTOptionsBuilder.newOptions()
			...
			.withOption('lengthMenu', [[20, 35, 50, 100, 1000000], [20, 35, 50, 100, 1000000]])
		...
		function render(data, type, full, meta) {
			switch (meta.col) {
				case 0:
					return CommonFunctions.FormatDateTime(full.AccessDateTimeString,3,null,null);
				case 1:
		...
		$scope.Filter = {
			loginId: '',
			operation: $scope.OperationFilterList[0],
			data: '',
			period: '7',
			mode: '1',
			access: '0',
		...
		$scope.getInetData = function () {
			vm.dtOptions
				...
				.withOption('ajax', function (data, callback, settings) {
					if ($scope.Filter.access == 1) {
						const params = { startDate: $scope.Filter.startDate, endDate: $scope.Filter.endDate, agentId: $scope.Filter.AgentToShow ? $scope.Filter.AgentToShow.AgentId : $scope.AgentAsCustomerInfo.CustomerID, start: data.start, length: data.length, search: data.search.value, sortBy: JSON.stringify(data.order), operationType: $scope.Filter.operation.value };
						$agentService.GetAgentInetSessionLog(params).then(function (response) {
						...
						if ($scope.Filter.mode == 1) {
							...
							const params = { agentId: $scope.Filter.AgentToShow ? $scope.Filter.AgentToShow.AgentId : $scope.AgentAsCustomerInfo.CustomerID, startDate: $scope.Filter.startDate, endDate: $scope.Filter.endDate, start: data.start, length: data.length, search: data.search.value, sortBy: JSON.stringify(data.order), operationType: $scope.Filter.operation.value };
							$agentService.GetCUSessionLog(params).then(function (response) {
								rawData = response.data.d.Data;
								callback({
									recordsTotal: rawData[0] ? rawData[0].Recordstotal : 0,
									recordsFiltered: rawData[0] ? rawData[0].Recordstotal : 0,
			...
			$agentService.GetIpInfo(ip).then(function (response) {
				$scope.IpInfo = JSON.parse(response);
				jQuery('#ModalIpInfo').modal('show');
			})
